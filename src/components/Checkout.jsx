              ...
              <React.Fragment key={s.num}>
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all"
                    style={step >= s.num ? { backgroundColor: '#00D4FF', color: '#000', boxShadow: '0 0 10px rgba(0,212,255,0.4)' } : { backgroundColor: 'rgba(255,255,255,0.05)', color: '#666' }}>
                    {step > s.num ? <Check size={16} /> : s.num}
                  </div>
                  <span className="text-sm font-medium hidden sm:block" style={step >= s.num ? { color: '#00D4FF' } : { color: '#666' }}>{s.label}</span>
                </div>
                {i < 2 && <div className="w-12 sm:w-20 h-0.5 transition-all" style={{ backgroundColor: step > s.num ? '#00D4FF' : 'rgba(255,255,255,0.05)' }} />}
              </React.Fragment>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            {step === 1 && (
              <div className="glass-card rounded-2xl p-6 space-y-6">
                <h2 className="text-xl font-bold text-text-primary flex items-center gap-2">
                  <MapPin size={20} style={{ color: '#00D4FF' }} /> Dados de Entrega
                </h2>
                ...
                <button onClick={() => setStep(2)} className="w-full py-4 text-black rounded-xl font-bold flex items-center justify-center gap-2 transition-all" style={{ backgroundColor: '#00D4FF', boxShadow: '0 0 12px rgba(0,212,255,0.5)' }}>
            ...
            {step === 2 && (
              <div className="glass-card rounded-2xl p-6 space-y-6">
                <h2 className="text-xl font-bold text-text-primary flex items-center gap-2">
                  <CreditCard size={20} style={{ color: '#00D4FF' }} /> Forma de Pagamento
                </h2>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    <button
                      ...
                      style={paymentMethod === m.id ? { borderColor: '#00D4FF', backgroundColor: 'rgba(0,212,255,0.05)', boxShadow: '0 0 10px rgba(0,212,255,0.2)' } : { borderColor: 'rgba(255,255,255,0.08)' }}
                    >
                      <m.icon size={24} className="mx-auto mb-2" style={{ color: paymentMethod === m.id ? '#00D4FF' : '#666' }} />
                      <p className="text-sm font-bold" style={{ color: paymentMethod === m.id ? '#00D4FF' : '#E0E0E0' }}>{m.label}</p>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setStep(3)} className="flex-1 py-4 text-black rounded-xl font-bold flex items-center justify-center gap-2 transition-all" style={{ backgroundColor: '#00D4FF', boxShadow: '0 0 12px rgba(0,212,255,0.5)' }}>
            ...
            {step === 3 && (
              <div className="glass-card rounded-2xl p-6 space-y-6">
                <h2 className="text-xl font-bold text-text-primary flex items-center gap-2">
                  <Check size={20} style={{ color: '#00E676' }} /> Revisão do Pedido
                </h2>
                <div className="p-4 rounded-xl space-y-2" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <div className="flex items-center gap-2 text-sm font-bold text-text-secondary">
                    <Truck size={16} style={{ color: '#00D4FF' }} /> Entrega
                  </div>
                ...
                <div className="p-4 rounded-xl">
                  <div className="flex items-center gap-2 text-sm font-bold text-text-secondary mb-1">
                    <CreditCard size={16} style={{ color: '#00D4FF' }} /> Pagamento
                  </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handleSubmit} disabled={loading} className="flex-1 py-4 text-black rounded-xl font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-50" style={{ backgroundColor: '#00E676', boxShadow: '0 0 12px rgba(0,230,118,0.5)' }}>
                    {loading ? (
          ...
          <div className="lg:col-span-1">
            <div className="glass-card rounded-2xl p-6 sticky top-40 space-y-4">
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {cart.map((item) => (
                  <div key={item.id} className="flex gap-3">
                    ...
                    <p className="text-sm font-bold" style={{ color: '#C6FF00' }}>R$ {(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
              <div className="space-y-2 pt-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                ...
                <div className="flex justify-between text-sm" style={{ color: '#C6FF00' }}>
                  <span>Frete</span><span className="font-semibold">{shipping === 0 ? 'Grátis' : `R$ ${shipping.toFixed(2)}`}</span>
                </div>
                {paymentMethod === 'pix' && (
                  <div className="flex justify-between text-sm" style={{ color: '#00E676' }}>
                ...
                <div className="flex justify-between text-lg font-bold text-text-primary pt-2 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                  <span>Total</span><span style={{ color: '#C6FF00' }}>R$ {paymentMethod === 'pix' ? (total * 0.95).toFixed(2) : total.toFixed(2)}</span>
                </div>
              </div>
              {shipping === 0 && (
                <p className="text-xs font-semibold text-center p-3 rounded-lg" style={{ color: '#C6FF00', backgroundColor: 'rgba(198,255,0,0.05)', border: '1px solid rgba(198,255,0,0.15)' }}>
                  Frete grátis!
                </p>
              )}
            </div>
