...
import { TrendingUp, Package, ShoppingCart, Users, ArrowUpRight, AlertTriangle, Clock, Upload, DollarSign, XCircle, CheckCircle } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    return () => { unsubOrders(); unsubProducts(); unsubClients(); };
  }, []);
  ...
  const stats = useMemo(() => {
    ...
    let pixCount = 0, cashCount = 0, cardCount = 0;

    const dailyRevenue = {};
    for (let i = 6; i >= 0; i--) {
      const d = new Date(today);
      d.setDate(d.getDate() - i);
      dailyRevenue[d.toDateString()] = { day: d.toLocaleDateString('pt-BR', { weekday: 'short' }), revenue: 0, orders: 0 };
    }
    ...
    const topProducts = Object.entries(productSales)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([name, quantity]) => ({ name: name.length > 20 ? name.substring(0, 17) + '...' : name, quantity }));
    ...
    return {
      revenueToday, revenueMonth, revenueWeek, totalOrders, ordersToday,
      lowStock, clients: clients.length,
      weeklyData, paymentData, topProducts,
      avgTicket, cancelRate
    };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card p-4 rounded-xl">
          ...
          <p className="text-xl font-black text-text-primary mt-1">{formatCurrency(stats.revenueToday)}</p>
        ...
        <div className="glass-card p-4 rounded-xl">
          ...
          <p className="text-xl font-black text-text-primary mt-1">{formatCurrency(stats.revenueMonth)}</p>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.weeklyData}>
                ...
                <Tooltip 
                  ...
                  contentStyle={{ backgroundColor: '#1A2238', border: 'none', borderRadius: 8, color: '#fff' }}
                />
        ...
        <div className="glass-card p-5 rounded-xl">
          ...
          {stats.paymentData.length > 0 ? (
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={stats.paymentData} cx="50%" cy="50%" innerRadius={40} outerRadius={60} dataKey="value" paddingAngle={4}>
            ...
            {stats.topProducts.length > 0 ? stats.topProducts.map((p, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <div className="flex-1 h-2 rounded-full bg-white/10 overflow-hidden">
                  <div className="h-full bg-neon-cyan" style={{ width: `${(p.quantity / stats.topProducts[0].quantity) * 100}%` }} />
                </div>
                <span className="text-xs text-text-primary font-medium w-8 text-right">{p.quantity}</span>
              </div>
            ...
            {orders.slice(0, 5).map(o => (
              <div key={o.id} className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                <div>
                  ...
                  <p className="text-xs text-text-dim">{new Date(o.date).toLocaleDateString('pt-BR')}</p>
                </div>
                <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${o.status === 'delivered' ? 'bg-green-500/20 text-green-400' : o.status === 'cancelled' ? 'bg-red-500/20 text-red-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
