...
const ProfitCalculator = ({ costPrice, profitMargin, price, onChange }) => {
  ...
  return (
    <div className="p-4 rounded-xl" style={{ backgroundColor: '#F8FAFC', border: '1px solid rgba(0,0,0,0.04)' }}>
      <p className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#64748B' }}>Calculadora de Lucro</p>
      <div className="grid grid-cols-3 gap-3">
        <div>
          <label className="text-xs font-medium" style={{ color: '#64748B' }}>Preço de Custo (R$)</label>
          <input
            ...
            style={{ backgroundColor: '#FFFFFF', border: '1px solid rgba(0,0,0,0.06)', color: '#1A2238' }}
            placeholder="0,00"
          />
        </div>
        <div>
          <label className="text-xs font-medium" style={{ color: '#64748B' }}>Margem (%)</label>
          <input
            type="number" step="0.1" min="0" max="1000"
            value={profitMargin === '' ? '' : profitMargin}
            onChange={e => handleChange('profitMargin', e.target.value)}
            className="w-full mt-1 px-3 py-2.5 rounded-lg text-sm outline-none text-center font-bold"
            style={{ backgroundColor: '#FFFFFF', border: '1px solid rgba(0,0,0,0.06)', color: profitMargin !== '' && parseFloat(profitMargin) >= 0 ? '#059669' : '#1A2238' }}
            placeholder="0"
          />
        </div>
        <div>
          <label className="text-xs font-medium" style={{ color: '#64748B' }}>Preço de Venda (R$)</label>
          <input
            type="number" step="0.01" min="0"
            value={price === '' ? '' : price}
      ...
      {costPrice !== '' && price !== '' && !isNaN(costPrice) && !isNaN(price) && costPrice > 0 && (
        <div className="mt-2 flex items-center gap-4 text-xs" style={{ color: '#64748B' }}>
          <span>Lucro: <strong style={{ color: profitMargin !== '' && parseFloat(profitMargin) > 0 ? '#059669' : '#EF4444' }}>R$ {(price - costPrice).toFixed(2)}</strong></span>
          {profitMargin !== '' && !isNaN(profitMargin) && (
            <span>Margem: <strong style={{ color: parseFloat(profitMargin) > 0 ? '#059669' : '#EF4444' }}>{profitMargin}%</strong></span>
          )}
        </div>
      )}
    </div>
